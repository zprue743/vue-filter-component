...
describe('FilterBuilder', () => {
  it('renders fields supplied by the consumer and adds a condition', async () => {
    ...
    const wrapper = mount(FilterBuilder, { props: { modelValue: createGroup(), fields } })

    expect(wrapper.find('button').text()).toContain('Add condition')
    await wrapper.get('button').trigger('click')

    const updated = wrapper.emitted('update:modelValue')?.[0]?.[0] as FilterGroup
  ...
  it('uses externally supplied static values', () => {
    ...
    const wrapper = mount(FilterBuilder, { props: { modelValue: filter, fields } })
  ...
  it('preserves the type of externally supplied select values', async () => {
    ...
    const updated = wrapper.emitted('update:modelValue')?.at(-1)?.[0] as FilterGroup
  ...
  it('emits edits, combinator changes, nested groups, and removals', async () => {
    const fields: FilterField[] = [{ key: 'name', label: 'Name', type: 'string' }]
    const filter = createGroup('and', [
    ...
    const edited = wrapper.emitted('update:modelValue')?.at(-1)?.[0] as FilterGroup
    ...
    const withOr = wrapper.emitted('update:modelValue')?.at(-1)?.[0] as FilterGroup
    ...
    const withGroup = wrapper.emitted('update:modelValue')?.at(-1)?.[0] as FilterGroup
    ...
    const removed = wrapper.emitted('update:modelValue')?.at(-1)?.[0] as FilterGroup
